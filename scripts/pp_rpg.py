...
def process_dirs(indirs, side="left", DELTA_MS=None):
    for indir in indirs: 
        ...
        # assert topics == sorted(['/davis_left/events', '/davis_left/image_raw', '/davis_right/events', '/davis_right/image_raw']) \
        #     or topics == sorted(['/prophesee/left/events', '/prophesee/right/events', '/camera/left/image_mono', '/camera/right/image_mono', '/kinect/depth_image'])
        ...
        try:
            imgtopics = [t for t in topics if side in t and "image" in t]
            if imgtopics:
                imgtopic = imgtopics[0]
                imgtopic_idx = topics.index(imgtopic)
        except Exception as e:
            print(f"Warning: Could not find image topic for side {side}: {e}")

        try:
            evtopics = [t for t in topics if side in t and "events" in t]
            if evtopics:
                evtopic = evtopics[0]
                evtopic_idx = topics.index(evtopic)
        except Exception as e:
            print(f"Warning: Could not find event topic for side {side}: {e}")

        if imgtopic is None and evtopic is None:
             raise ValueError(f"Could not find any relevant topics for side {side} in {topics}")
        ...
        if imgtopic:
            ...
            assert (H == 180 and W == 240) or (H == 260 and W == 346) or (H > 0 and W > 0)
        else:
            print("Warning: No image topic found, skipping H/W reading.")
        ...
        if not os.path.exists(imgdirout):
            ...
            if imgtopic and bag.get_message_count(topics[imgtopic_idx]) == len(img_list_undist):
        ...
        if seq != "simulation_3planes" and imgtopic:
            if side == "left":
                intrinsics = [196.63936292910697, 196.7329768429481, 105.06412666477927, 72.47170071387173, 
                            -0.3367326394292646, 0.11178850939644308, -0.0014005281258491276, -0.00045959441440687044]
            else:
        ...
        # writing pose to file
        posetopic = None
        possible_pose_topics = ["/optitrack/davis_stereo", f"/davis/{side}/pose", "/gt/pose"]
        for t in possible_pose_topics:
            if t in topics:
                posetopic = t
                break
        
        if posetopic:
             # TODO: T_marker_cam0 and T_cam0_cam1 might need adjustment for different datasets
             if posetopic == "/optitrack/davis_stereo":
            ...
            print(f"Warning: No pose topic found in {topics}")
            T_marker_cam0 = np.eye(4)
            T_cam0_cam1 = np.eye(4)
        ...
        start_times = []
        if tss_gt_us: start_times.append(tss_gt_us[0])
        if tss_imgs_us: start_times.append(tss_imgs_us[0])
        if t0_evs is not None: start_times.append(t0_evs)
        
        if not start_times:
            print("Error: No timestamps found from any source!")
            continue

        t0_us = np.min(start_times)
